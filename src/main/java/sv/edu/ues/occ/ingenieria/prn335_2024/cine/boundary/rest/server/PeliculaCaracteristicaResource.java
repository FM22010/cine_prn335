...
public class PeliculaCaracteristicaResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<PeliculaCaracteristica> encontrados =pcBean.findRange(fristResult, maxResult);
                Long total=(long) pcBean.count();
    ...
    public Response findById(@PathParam("id") Integer id){
        if(id!=null){
            try {
                PeliculaCaracteristica encontrado=pcBean.findById(id);
                if(encontrado!=null){
    ...
    public Response create(PeliculaCaracteristica peliculaCaracteristica, @Context UriInfo uriInfo){
        if(peliculaCaracteristica != null && peliculaCaracteristica.getIdPeliculaCaracteristica()==null){
            try {
                pcBean.create(peliculaCaracteristica);
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+peliculaCaracteristica).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            PeliculaCaracteristica peliculaCaracteristica = pcBean.findById(id);
            if (peliculaCaracteristica!= null) {
                pcBean.getEntityManager().remove(peliculaCaracteristica);
