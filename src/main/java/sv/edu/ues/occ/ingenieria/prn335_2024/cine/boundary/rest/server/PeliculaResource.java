...
public class PeliculaResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<Pelicula> encontrados =peBean.findRange(fristResult, maxResult);
                Long total=(long) peBean.count();
    ...
    public Response create(Pelicula pelicula, @Context UriInfo uriInfo){
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+pelicula).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            Pelicula pelicula = peBean.findById(id);
            if (pelicula!= null) {
                peBean.getEntityManager().remove(pelicula);
