...
public class ReservaDetalleResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<ReservaDetalle> encontrados =redBean.findRange(fristResult, maxResult);
                Long total=(long) redBean.count();
    ...
    public Response findById(@PathParam("id") Integer id){
        if(id!=null){
            try {
                ReservaDetalle encontrado=redBean.findById(id);
                if(encontrado!=null){
    ...
    public Response create(ReservaDetalle reservaDetalle, @Context UriInfo uriInfo){
        if(reservaDetalle != null && reservaDetalle.getIdReservaDetalle()==null){
            try {
                redBean.create(reservaDetalle);
                if(reservaDetalle.getIdReservaDetalle()!=null){
                    UriBuilder uriBuilder=uriInfo.getAbsolutePathBuilder();
                    uriBuilder.path(String.valueOf(reservaDetalle.getIdReservaDetalle()));
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+reservaDetalle).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            ReservaDetalle reservaDetalle = redBean.findById(id);
            if (reservaDetalle!= null) {
                redBean.getEntityManager().remove(reservaDetalle);
