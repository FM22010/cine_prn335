...
public class TipoAsientoResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<TipoAsiento> encontrados =taBean.findRange(fristResult, maxResult);
    ...
    public Response create(TipoAsiento tipoAsiento, @Context UriInfo uriInfo){
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+tipoAsiento).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            TipoAsiento tipoAsiento = taBean.findById(id);
            if (tipoAsiento!= null) {
                taBean.getEntityManager().remove(tipoAsiento);
