...
public class SucursalResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<Sucursal> encontrados =suBean.findRange(fristResult, maxResult);
                Long total=(long) suBean.count();
    ...
    public Response findById(@PathParam("id") Integer id){
        if(id!=null){
            try {
                Sucursal encontrado=suBean.findById(id);
                if(encontrado!=null){
    ...
    public Response create(Sucursal sucursal, @Context UriInfo uriInfo){
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+sucursal).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            Sucursal sucursal = suBean.findById(id);
            if (sucursal!= null) {
                suBean.getEntityManager().remove(sucursal);
