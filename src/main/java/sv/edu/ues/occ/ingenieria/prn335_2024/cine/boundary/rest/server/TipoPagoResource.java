...
public class TipoPagoResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<TipoPago> encontrados =tpaBean.findRange(fristResult, maxResult);
                Long total=(long) tpaBean.count();
    ...
    public Response findById(@PathParam("id") Integer id){
        if(id!=null){
            try {
               TipoPago encontrado=tpaBean.findById(id);
                if(encontrado!=null){
    ...
    public Response create(TipoPago tipoPago, @Context UriInfo uriInfo){
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+tipoPago).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            TipoPago tipoPago = tpaBean.findById(id);
            if (tipoPago!= null) {
                tpaBean.getEntityManager().remove(tipoPago);
