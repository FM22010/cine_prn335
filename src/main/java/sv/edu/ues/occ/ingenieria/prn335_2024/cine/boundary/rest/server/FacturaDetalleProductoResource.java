...
public class FacturaDetalleProductoResource implements Serializable {
    @Inject
    FacturaDetalleProductoBean fdprBean;
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<FacturaDetalleProducto> encontrados =fdprBean.findRange(fristResult, maxResult);
                Long total=(long) fdprBean.count();
    ...
    public Response findById(@PathParam("id") Integer id){
        if(id!=null){
            try {
                FacturaDetalleProducto encontrado=fdprBean.findById(id);
                if(encontrado!=null){
    ...
    public Response create(FacturaDetalleProducto facturaDetalleProducto, @Context UriInfo uriInfo){
        if(facturaDetalleProducto != null && facturaDetalleProducto.getIdFacturaDettalleProducto()==null){
            try {
                fdprBean.create(facturaDetalleProducto);
                if(facturaDetalleProducto.getIdFacturaDettalleProducto()!=null){
                    UriBuilder uriBuilder=uriInfo.getAbsolutePathBuilder();
                    uriBuilder.path(String.valueOf(facturaDetalleProducto.getIdFacturaDettalleProducto()));
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+facturaDetalleProducto).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            FacturaDetalleProducto facturaDetalleProducto = fdprBean.findById(id);
            if (facturaDetalleProducto!= null) {
                fdprBean.getEntityManager().remove(facturaDetalleProducto);
