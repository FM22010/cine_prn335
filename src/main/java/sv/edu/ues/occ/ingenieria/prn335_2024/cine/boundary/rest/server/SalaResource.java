...
public class SalaResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if(fristResult >=0 && maxResult >0 && maxResult<=20){
                List<Sala> encontrados =saBean.findRange(fristResult, maxResult);
                Long total=(long) saBean.count();
    ...
    public Response findById(@PathParam("id") Integer id){
        if(id!=null){
            try {
                Sala encontrado=saBean.findById(id);
                if(encontrado!=null){
    ...
    public Response create(Sala sala, @Context UriInfo uriInfo){
        ...
        return Response.status(422).header("Wrong-Parameter","tiposala:"+sala).build();
    }
    ...
    public Response delete(@PathParam("id") Integer id) {
        ...
        try {
            Sala sala = saBean.findById(id);
            if (sala!= null) {
                saBean.getEntityManager().remove(sala);
